...
export class ThemePacker {
  ...
  private async getAllDeps(cssPath: string, root?: string) {
    ...
    const css = await fsP.readFile(cssPath, { encoding: 'utf-8' })
    ...
    for (const m of match) {
      ...
      if (line) {
        ...
        if (importPath) {
          imports.push(path.resolve(path.dirname(cssPath), importPath[2]))
          const imported = await this.getAllDeps(importPath[2], path.dirname(cssPath))
